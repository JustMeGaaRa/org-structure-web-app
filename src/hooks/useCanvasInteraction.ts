...
export function useCanvasInteraction(
  ...
  const handleStartDragCard = (e: React.PointerEvent, cardId: string) => {
    ...
    if (toolMode !== "select" || (e.pointerType === "mouse" && e.button !== 0))
      return;
    e.stopPropagation();
  ...
  const handlePointerMove = useCallback(
    ...
    [setCards, setTracks, setTransform, deleteZoneRef, handleZoom, canvasRef],
  );
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    ...
    // Pan Mode
    if (toolMode === "pan" || (e.pointerType === "mouse" && e.button === 1)) {
      setIsPanning(true);
